...
webconfig_error_t webconfig_set(webconfig_t *config, webconfig_subdoc_data_t *data)
{
    ...
    if ((data->descriptor & webconfig_data_descriptor_decoded) == webconfig_data_descriptor_decoded) {
        ...
        if ((err = doc->encode_subdoc(config, data)) != webconfig_error_none) {
            wifi_util_error_print(WIFI_CTRL, "%s:%d: [Onewifi crash] encode_subdoc failed\n", __FUNCTION__, __LINE__);
        } else {
            wifi_util_dbg_print(WIFI_CTRL, "%s:%d: [Onewifi crash] encode_subdoc success, encoded.raw=%p\n",
        ...
        if ((data->descriptor = webconfig_data_descriptor_encoded)
            ...
            wifi_util_error_print(WIFI_CTRL, "%s:%d: [Onewifi crash] apply_data failed\n", __FUNCTION__, __LINE__);
            err = webconfig_error_apply;
        } else {
            wifi_util_dbg_print(WIFI_CTRL, "%s:%d: [Onewifi crash] apply_data success\n", __FUNCTION__, __LINE__);
        }
    }
